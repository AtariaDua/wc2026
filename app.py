...
st.caption("Версия 13.2: Автоматические аналитические комментарии к экспрессу")
...
if not live_data:
    ...
    if market_mode == "🔥 Сливки Дня":
        ...
        if len(express_pool) >= 2:
            ...
            # --- АВТОМАТИЧЕСКИЙ АНАЛИТИЧЕСКИЙ КОММЕНТАРИЙ ИИ ---
            st.info("📋 **Обоснование ИИ-синдиката:**\n"
                    f"* 🎯 Позиция 1 ({leg1['market']}): Букмекер выставил кэф {leg1['odds']} под влиянием прогруза толпы, проигнорировав базовые тактические метрики флангов. Математическая симуляция видит здесь чистую валуйную ошибку линии.\n"
                    f"* 🎯 Позиция 2 ({leg2['market']}): Стилистическая матрица команд подтверждает жесткое несовпадение атакующих кроссов и высоты защиты, что дает нам перевес в +{leg2['edge']:.1f}% над текущей котировкой БК.")
        else:
